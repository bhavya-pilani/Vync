...
const VideoCard = (props: Props) => {
  ...
  // 🟢 2. Trigger it on the first client render
  useEffect(() => {
    setIsMounted(true);
  }, []);

  // 🟢 3. If it's rendering on the server, return a safe skeleton UI
  if (!isMounted) {
    return (
      <div className="w-full aspect-[16/10] bg-[#171717] animate-pulse rounded-xl border-[1px] border-[#252525]"></div>
    );
  }
    ...
    >
      <div className=" group overflow-hidden cursor-pointer bg-[#171717] relative border-[1px] border-[#252525] flex flex-col rounded-xl">
        ...
        >
          ...
          {/* Note: I corrected the typo 'gap-7-2' to 'gap-y-2' below */}
